...
\usepackage{siunitx}            % SI units  (https://ctan.org/pkg/siunitx
...
% \author{Patricia García-Berlanga\thanks{Universidad Politécnica de Cartagena, patricia.garciab@edu.upct.es} \and Juan Zapata\thanks{Universidad Politécnica de Cartagena, juan.zapata@upct.es} \and  Pablo Hernández-Cerdán\thanks{Vasomaly, pablo.hernandez@vasomaly.com} \and Juan Martínez-Alajarín\thanks{Universidad Politécnica de Cartagena, juanc.martinez@upct.es}}

%  Buenas tardes.

% He revisado las guidelines de la revista y estas son las pequeñas modificaciones que debemos hacer para seguir todas las recomendaciones:

%     En la guía se especifica: "Title page should include paper title, author(s) full name and affiliation, corresponding author(s) names complete affiliation/address, along with phone, fax and email.". Por lo tanto, nos faltaría completar las afiliaciones según en formato que nos envió Juan y añadir también número de teléfono y email. Por cierto, Pablo me contestó que su nombre para publicar es Pablo Hernandez-Cerdan, sin tildes, y que pusiéramos su correo público que es pablo.hernandez.cerdan@outlook.com.

%     El abstract no debe tener más de 250 palabras (las sobrepasamos un poco) y además debe estar estructurado con diferentes subtítulos, que pueden ir dentro del mismo párrafo en línea y en negrita. Los subtítulos recomendados son Introduction/Objective, Methods, Results, Discussion y Conclusion, aunque indica que pueden variar.

%      Se piden 6-8 keywords, preferiblemente coincidentes con los temas del título, que aparezcan justo debajo del resumen.

%     Hay que añadir una sección llamada Ethics Approval and Consent to Participate, donde se confirme que el estudio cumple las normas éticas. En este caso, como el dataset es público y estña anonimizado, no se requiere consentimiento, pero es conveniente aclararlo. Por ejemplo, podría escribirse esto: "This study was conducted using the publicly available ISLES 2022 dataset, which was collected and distributed in accordance with relevant ethical standards and anonymized prior to release. Therefore, additional ethical approval and informed consent were not required."

%     Es necesario añadir una sección llamada Conflict of Interest antes de las referencias, en la que se incluya una confirmación de que efectivamente no hay conflicto de intereses. El ejemplo que aparece es “The author(s) confirm(s) that there is no conflict of interest related to the manuscript.”. Además, hay que rellenar y entregar también un Disclosure Form (adjunto), aunque esto es más bien documentación que hay que entregar.


% En cuanto al tipo de documento, aparecen diferentes clasificaciones (https://www.benthamscience.com/pages/author-guidelines). La que más se ajusta al nuestro es Research Article, pero se especifica que deben incluir 75 o más referencias. Como solo tenemos 25, creo que también encajaría en Short Communications ("Short Communications are concise articles that present significant findings or innovative ideas in a brief and focused format. They are intended to communicate preliminary results, novel methods, or theoretical advancements that may not require the extensive background, methodology, and discussion of a full-length research article. Short communications should be 2000-3000 words with 25 or more references excluding figures, structures, photographs, schemes, tables, etc."). No sé qué opináis al respecto.

% Teniendo todo esto en cuenta, la documentación a entregar sería:

%     Manuscrito principal, en Word o LaTeX.
%     Figuras y tablas por separado. Las figuras en TIFF, PNG o JPEG, y las tablas en Microsoft Word table format.
%     El Disclosure Form.
%     Una Copyright Letter (la adjunto también).


% Esto sería todo. Una vez que se hagan esos cambios y se rellenen los documentos pertinentes ya podría iniciarse el proceso. ¿Hemos recibido contestación respecto a los plazos?

\author{
  Patricia García-Berlanga$^{1}$\thanks{Corresponding author: patricia.garciab@edu.upct.es} \and
  Juan Zapata$^{1}$ \and 
  Juan Martinez-Alajarin$^{1}$ \and
  Pablo Hernandez-Cerdan$^{2}$
  }

\date{
  \small $^{1}$Universidad Politécnica de Cartagena, \\
  \small member of European University of Technology EUT+,\\
  \small ETSII, C/ Dr. Fleming, s/n, 30202 Cartagena, Spain\\[0.5em]
  \small $^{2}$Vasomaly, Avenida Fontes 15, Torre-Pacheco,\\ 
  \small 30700, Murcia, Spain\\[1em]
  %\today
}
...
\noindent\textbf{Keywords:} Stroke lesion segmentation, Non-contrast MRI,  Automated diagnosis, Clinical deployment, FLAIR MRI, Medical image analysis.
...
The 3D U-Net training was automatically stopped after 24 epochs due to the early stopping criterion. %, as shown in Figure~\ref{fig:unet_loss_curve},which depicts the training loss curve. 
%The validation Dice coefficient increased steadily before reaching a stable value around epoch 20. 
...
%
The nnU-Net model automatically configured its preprocessing, patch size, and training schedule. It reached optimal validation performance after 141 epochs. %, as indicated by the decreasing loss curve and the rising pseudo-Dice score, shown in Figure~\ref{fig:nnu-net_loss_curve}. 
...
The YOLOv8-seg model was trained for 100 epochs %, with training and validation loss curves shown in Figure~\ref{fig:yolov8_loss_curve}. The loss curves 
...
\section*{Ethics Approval and Consent}\label{sec:ethics}

This study was conducted using the publicly available ISLES 2022 dataset, which was collected and distributed in accordance with relevant ethical standards and anonymized prior to release. No new patient data were collected, and all analyses were conducted in accordance with relevant guidelines and regulations. Therefore, additional ethical approval and informed consent were not required.

\section*{Conflict of Interest}\label{sec:conflict_of_interest}
The authors confirm that there is no conflict of interest related to the manuscript.

\section*{Acknowledgments}
The authors would like to thank the providers of the ISLES 2022 dataset for making this valuable resource publicly available, which greatly facilitated this research. We also acknowledge the support of our institution's computational resources that enabled the training and evaluation of the deep learning models presented in this study. The authors would also like to acknowledge Vasomaly for their technical support and valuable insights that contributed to the successful completion of this work
% agradecer a algun proyecto de financiacion si es necesario
% \section*{Funding}
This work was supported by the Spanish Ministry of Science, Innovation and Universities under grant PID2021-128009OB-C33 funded by MCIN/AEI/10.13039/501100011033/FEDER, UE, which provided essential resources for data processing and model training.
