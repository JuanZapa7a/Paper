...
All experiments were conducted using Python-based frameworks for deep learning and medical image processing. Training was performed on a GPU-equipped environment (NVIDIA Tesla T4) to ensure efficient execution of 3D convolutional networks. The implementations relied on TensorFlow/Keras for U-Net \cite{U-NetConvolutionalNetworks2015}, nnU-Net \cite{Isensee2021} for the self-configuring model, and the Ultralytics framework for YOLOv8 \cite{ultralyticsYOLOv8RealtimeObject2024}. The setup ensured full reproducibility across experiments. 
